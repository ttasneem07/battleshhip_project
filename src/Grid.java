...
public class Grid extends JPanel implements MouseListener, MouseMotionListener {
     ...
     * @param visible true if all the ships should be shown, as is the case with the
     *                human's board
     */
    ...
    /**
     * Tell the grid the shape of the object we're placing, or null if we don't have
     * a hover shape.
     * 
    ...
    /**
     * Callback for repainting the contents of the window. Called automatically by
     * the GUI framework.
     * 
    ...
    public void paint(Graphics g) {
        ...
        g2d.scale((double) dim.width / cols / CELL_SIZE, (double) dim.height / rows / CELL_SIZE);
    ...
    private void drawCircle(Graphics g, int row, int col) {
        ...
        g.fillOval(col * CELL_SIZE + offset, row * CELL_SIZE + offset, CIRCLE_SIZE, CIRCLE_SIZE);
    }
        ...
        for (int i = 0; i < board.getNumberOfShips(); i++) {
            ...
            if (ships[i].isHorizontal()) {
                for (int col = leftCol; col < leftCol + length; col++) {
                    if (board.hasBeenHit(topRow, col))
                        drawCircle(g, topRow, col);
                }
            } else {
                for (int row = topRow; row < topRow + length; row++) {
                    if (board.hasBeenHit(row, leftCol))
                        drawCircle(g, row, leftCol);
                }
            }
        }
    }
    ...
    /**
     * Callback for when a mouse button is pressed on the grid. We convert it to row
     * and column, and notify the GUI object.
     * 
     * @param e mouse event
     */
    ...
    /**
     * Callback for when the mouse is moved in the window. We remember the mouse
     * location to draw the highlight of the ship being placed.
     * 
     * @param e mouse event
     */
    ...
    public Dimension getPreferredSize() {
        return new Dimension(board.getNumberOfColumns() * CELL_SIZE, board.getNumberOfRows() * CELL_SIZE);
    }
    ...
    public Dimension getMinimumSize() {
        return new Dimension(board.getNumberOfColumns() * MIN_CELL_SIZE, board.getNumberOfRows() * MIN_CELL_SIZE);
    }
