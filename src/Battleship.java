...
public class Battleship {
    ...
    /**
     * True if all the human's ships have been placed (i.e., when we're playing the
     * game.)
     */
    ...
    /**
     * Constructor - sets up - boards for human and computer - panels for displaying
     * these boards - an AI program for the computer - the GUI
     * 
     * @param seed the random seed that is used for the computer's AI; if it's 0,
     *             the random sequence is unpredictable and not repeatable.
     */
    ...
    /**
     * Just like the one-parameter constructor, but don't supply a seed to the game
     * AI.
     */
    public Battleship() {
        humanBoard = new Board(ROWS, COLS, NUMBER_OF_SHIPS);
    ...
    public void selectHumanGridSquare(int row, int column) {
        ...
        if (!donePlacingShips) {
            if (!humanBoard.addShip(SHIP_LENGTHS[shipIndex], horizontal, row, column)) {
    ...
    /**
     * Checks the results of firing at a position and lets the GUI know if either
     * party has won
     * 
