...
public class BattleshipGUI extends JFrame implements ActionListener {
    ...
    /**
     * Callback from the Grid objects, for when the button gets pressed. This is
     * really an example of cyclic dependency, but, we'd need to use some more
     * object-oriented features to do a good job fixing it.
     * 
    ...
    public void gridPress(Grid grid, int row, int column) {
        if (grid == humanGrid) {
            ...
            /*
             * if (firstComputerShot) { firstComputerShot = false; try { Thread.sleep(5000);
